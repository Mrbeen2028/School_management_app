...
class TeacherHomeworkDiaryPage extends StatefulWidget {
  final String teacherName; // Comes from login

  const TeacherHomeworkDiaryPage({super.key, required this.teacherName});

  @override
  _TeacherHomeworkDiaryPageState createState() =>
      _TeacherHomeworkDiaryPageState();
}

class _TeacherHomeworkDiaryPageState extends State<TeacherHomeworkDiaryPage> {
  ...
  Future<List<Map<String, dynamic>>> _fetchHomework(String className) async {
    final ref =
    FirebaseDatabase.instance.ref().child("homework").child(className);
    ...
    if (snapshot.exists && snapshot.value is Map) {
      ...
      allDates.forEach((dateKey, entriesMap) {
        if (!showFullHistory &&
            dateKey != DateFormat('yyyy-MM-dd').format(selectedDate)) return;
        ...
        if (entriesMap is Map) {
          ...
          entries.forEach((key, value) {
            if (value is Map) {
              ...
              final createdByName = value['createdByName'] ?? '';

              // Teacher should see Admin + their own homework only
              if (createdBy == 'admin' || createdBy == widget.teacherName) {
                list.add({
                  "id": key,
  ...
  Future<void> _uploadHomework() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (_editingId != null) {
        await ref.child(_editingId!).update({
  ...
  Future<void> _deleteHomework(String id, String dateKey) async {
    final ref = FirebaseDatabase.instance
        .ref()
        .child("homework")
        .child(selectedClass)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text("📘 Teacher Homework Diary",
